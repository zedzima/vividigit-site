...
function syncSidebarOverlay() {
    const navOpen = !!sidebarNav?.classList.contains('open');
    const actionOpen = !!sidebarAction?.classList.contains('open');
    const isOpen = navOpen || actionOpen;
...
btnMenu?.addEventListener('click', () => {
    const shouldOpen = !sidebarNav?.classList.contains('open');
    sidebarNav?.classList.toggle('open', shouldOpen);
    sidebarAction?.classList.remove('open');
    syncSidebarOverlay();
});

btnAction?.addEventListener('click', () => {
    const shouldOpen = !sidebarAction?.classList.contains('open');
    sidebarAction?.classList.toggle('open', shouldOpen);
    sidebarNav?.classList.remove('open');
    syncSidebarOverlay();
});
...
function isContactHref(href) {
    ...
    const normalized = href.replace(/^\.\//, '').replace(/^\/+/, '').toLowerCase();
...
function openContactSidebar() {
    ...
    const contactTab = document.querySelector('.sidebar-tabs .sidebar-tab[data-tab="contact"]');
    if (contactTab && !contactTab.classList.contains('active')) {
        contactTab.click();
    }

    const sidebarContent = document.querySelector('.sidebar-action .sidebar-content');
    if (!sidebarContent) return;

    let contactWidget = null;
    const contactPanel = sidebarContent.querySelector('.sidebar-tab-content[data-tab-content="contact"]:not(.hidden)');
    if (contactPanel) {
...
function addFromPricingBlock() {
    const simpleBtn = document.querySelector('.pricing-simple-cta');
    if (simpleBtn) {
        simpleBtn.click();
        return true;
    }

    const packageBtn = document.querySelector('.pricing-package .btn');
    if (packageBtn) {
        packageBtn.click();
        return true;
    }

    const tier = document.querySelector('.pricing-tier');
    if (tier) {
        tier.click();
        return true;
    }
...
function addFromTaskPicker() {
    const firstChecked = document.querySelector('.task-select-cb:checked');
    const cb = firstChecked || document.querySelector('.task-select-cb');
    if (!cb) return false;

    const item = cb.closest('.task-picker-item');
    const activeTier = item ? item.querySelector('.tier-btn.active') : null;
    const billing = {
        oneTime: cb.dataset.oneTime === 'true',
        monthly: cb.dataset.monthly === 'true',
        yearly: cb.dataset.yearly === 'true'
    ...
    // Force re-add for already selected door-opener task.
...
function openOrderSidebar() {
    ...
    const orderTab = document.querySelector('.sidebar-tabs .sidebar-tab[data-tab="order"]');
    if (orderTab && !orderTab.classList.contains('active')) {
        orderTab.click();
    }

    const cartWidget = document.getElementById('cartItems');
    if (cartWidget) {
        cartWidget.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
}
...
function normalizeCtaButtons() {
    document.querySelectorAll('a.btn, button.btn').forEach(function(btn) {
        ...
        const href = btn.tagName === 'A' ? (btn.getAttribute('href') || '').trim() : '';
...
function setupGlobalCtaActions() {
    ...
    document.addEventListener('click', function(e) {
        ...
        const label = getBtnLabel(btn);
        const href = btn.tagName === 'A' ? (btn.getAttribute('href') || '').trim() : '';
        const inPricingBlock = !!btn.closest('.pricing-block');
...
function initSidebarShare() {
    const shareWidget = document.getElementById('sidebarShare');
    if (!shareWidget) return;

    const contactPanel = document.querySelector('.sidebar-tab-content[data-tab-content="contact"]');
    if (contactPanel) {
        contactPanel.appendChild(shareWidget);
    } else {
        let lastContactWidget = null;
        document.querySelectorAll('.sidebar-content .widget').forEach(function(widget) {
            const hasEmail = !!widget.querySelector('input[type="email"], input[name="email"]');
            const hasMessage = !!widget.querySelector('textarea[name="message"], textarea.form-input');
            const hasSubmit = !!widget.querySelector('button.btn-primary.btn-full');
    ...
    const pageUrl = canonicalHref.trim() || window.location.href;
    const pageTitle = document.title || 'VIVIDIGIT';
    const encodedUrl = encodeURIComponent(pageUrl);
    const encodedTitle = encodeURIComponent(pageTitle);

    shareWidget.querySelectorAll('.sidebar-share-btn[data-share]').forEach(function(btn) {
        const network = (btn.dataset.share || '').toLowerCase();
